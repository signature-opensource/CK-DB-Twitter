...
{
    public abstract partial class UserTwitterTable
    {
        /// <summary>
        ...
        public UCLResult CreateOrUpdateTwitterUser( ISqlCallContext ctx, int actorId, int userId, IUserTwitterInfo info, UCLMode mode = UCLMode.CreateOrUpdate )
        {
            return UserTwitterUCL( ctx, actorId, userId, info, mode );
        }
        ...
        public LoginResult LoginUser( ISqlCallContext ctx, IUserTwitterInfo info, bool actualLogin = true )
        {
            var mode = actualLogin
                        ? UCLMode.UpdateOnly | UCLMode.WithActualLogin
                        : UCLMode.UpdateOnly | UCLMode.WithCheckLogin;
            var r = UserTwitterUCL( ctx, 1, 0, info, mode );
        ...
        public abstract void DestroyTwitterUser( ISqlCallContext ctx, int actorId, int userId );
        ...
        protected abstract UCLResult UserTwitterUCL(
            ISqlCallContext ctx,
            int actorId,
            int userId,
