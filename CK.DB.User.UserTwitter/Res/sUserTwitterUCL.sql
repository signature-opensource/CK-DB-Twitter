-- SetupConfig: {}
--
-- @TwitterAccountId can not be null. This is the key that identifies a Twitter user.
--
...
--
create procedure CK.sUserTwitterUCL
(
	@ActorId int,
	@UserId int /*input*/output,
	@TwitterAccountId varchar(36), 
...
begin
	...
	if @TwitterAccountId is null throw 50000, 'Argument.NullTwitterAccountId', 1;
	...
	begin
		...
		begin
			...
			-- Unique constraint on TwitterAccountId will detect any existing UserId/TwitterAccountId clashes.
	...
	begin
		...
		begin
			...
			else if @UserId <> @ActualUserId throw 50000, 'Argument.UserIdAndTwitterIdMismatch', 1;
			...
			update CK.tUserTwitter set 
					LastLoginTime = LastLoginTime 
				where UserId = @ActualUserId and TwitterAccountId = @TwitterAccountId;
	...
	begin
        ...
        begin
		    exec CK.sAuthUserOnLogin 'Twitter', @LastLoginTime, @UserId, @ActualLogin, @Now, @LoginFailureCode output, @LoginFailureReason output;  
            if @ActualLogin = 1 and @LoginFailureCode is null
            begin
			    update CK.tUserTwitter set LastLoginTime = @Now
                    where UserId = @UserId and TwitterAccountId = @TwitterAccountId;
            end
        end
	end
